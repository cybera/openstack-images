...
TEMP_ID=`glance image-create --disk-format qcow2 --container-format bare --file Fedora-Cloud-Base-22-20150521.x86_64.qcow2 --name TempFedoraImage | grep id | awk ' { print $4 }'`
...
glance image-update --name "Fedora 22"  --property description="Built on `date`" --property image_type='image' --remove-property base_image_ref --remove-property image_location --remove-property instance_uuid --remove-property owner_id --remove-property user_id "${IMAGE_ID}"
